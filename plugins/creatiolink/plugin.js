CKEDITOR.plugins.add('creatiolink', {
	...
	init: function (editor) {
		...
		CKEDITOR.plugins.creatiolink = {
			...
			getOffsetForLinkComponent: function (editor, currentLinkComponentWidth, currentLinkComponentHeight) {
				const text = editor.getSelection().getSelectedText().toString();
				let element;
				if (!text) {
				...
				x += obj.offsetLeft;
				y += obj.offsetTop;
				let leftMostPossiblePosition = document.documentElement.clientWidth - currentLinkComponentWidth;
				if (x > leftMostPossiblePosition) {
					x = leftMostPossiblePosition - 8;
				}
				let heightMostPossiblePosition = document.documentElement.clientHeight - currentLinkComponentHeight;
				if (y > heightMostPossiblePosition) {
					y = heightMostPossiblePosition - 8;
				}
				if (!text) {
					element.remove();
				}
				return { leftPosition: x + 'px', topPosition: y + 'px' };
			},
